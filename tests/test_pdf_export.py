...
def test_32_cjk_range_boundary_does_not_false_positive_on_japanese_kana_or_hangul():
    ...
    ошибочно определяться как 'китайский' (has_cjk). Уточнение (проверено
    эмпирически): сам шрифт WenQuanYiMicroHei глифы каны и хангыля
    СОДЕРЖИТ — дело не в отсутствии глифов в шрифте, а в том, что
    диапазон-классификатор в _generate_pdf() намеренно проверяет только
    \\u4E00-\\u9FFF, поэтому кана/хангыль всё равно роутится в латинскую
    ветку (FiraSans) и даёт пустые квадраты — по причине классификации,
    а не покрытия шрифта. Здесь фиксируем сам факт: чисто кана-строка не
    должна попадать в CJK-ветку классификации (только проверка диапазона
    на уровне текста, без обращения к приватному состоянию функции —
    косвенно, через то что рендер не падает и это остаётся заботой
    будущего цикла, если понадобится японский/корейский)."""
