...
_PROTECT_PATTERNS = [
    ...
    (r"(?<!\d)\d+(?:[,.]\d+)?\s*(?:%|K\+?|M\+?|B\+?|k\+?|m\+?|\+)(?![A-Za-z\d])", "NUM_METRIC"),
    # ID: 7+ цифр подряд
    (r"\b\d{7,}\b", "ID_NUM"),
...
def _extract_facts(text):
    ...
    Цикл E2: то же правило распространено на буллет-маркер "• " —
    если текст начинается с символа \u2022 (с ведущими/последующими
    пробелами), позиция сразу после маркера тоже считается началом
    предложения. Подтверждено на реальных данных (resume_ua.docx): без
    этого исправления первое слово буллета после легитимной замены
    синонимом ("Впровадив"→"Запровадив") ложно ловилось как "invented
    fact", т.к. находится не в позиции 0, а в позиции 2+ (после "• ").
    Другие маркеры списков ("-", "*", "1.") сознательно НЕ обрабатываются
    в этом цикле — только "•", единственный подтверждённый на данных
    случай (см. отчёт Цикла E2).
    """
    facts = set()
    ...
    # Цикл E2: буллет-маркер "• " в начале текста тоже открывает
    # "предложение" — первое слово после него не факт.
    bullet_match = re.match(r"^\s*\u2022\s*", text)
    if bullet_match:
        sentence_start_positions.add(bullet_match.end())
