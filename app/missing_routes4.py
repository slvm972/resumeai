...
def _apply_improved_text_to_docx(original_bytes, improved_text, item_ids):
    ...
    while i + 1 < len(parts):
        ...
        # python-docx в два <w:br/> подряд, что даёт визуальный эффект
        # "лишнего" маркера буллета и увеличивает высоту параграфа (в т.ч.
        # растягивание документа на лишнюю страницу). Тот же приём уже
        # применяется в _parse_ai_response — здесь это финальный рубеж
        # перед записью в DOCX, страхует независимо от того, где именно
        # в пайплайне мог появиться двойной перенос.
        text = re.sub(r"\n{2,}", "\n", text)
