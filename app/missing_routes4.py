...
# ---------------------------------------------------------------------------
# Отбрасывание пустых "хвостов"-буллетов внутри параграфа (Cycle B)
# ---------------------------------------------------------------------------

# Символы буллет-маркеров, которые могут стоять в начале "логической строки"
# внутри параграфа (то есть после <w:br/>, который python-docx переводит
# в "\n" в para.text): •, -, *, а также типографские варианты маркеров.
_BULLET_STRIP_RE = re.compile(r'^[\s\-\*•\u2022\u25CF\u25E6\u2043\u2219]+')


def _strip_empty_bullet_lines(text):
    """
    Убрать из текста параграфа "логические строки" (разделённые "\\n" —
    в исходном XML это <w:br/> внутри одного <w:p>, не отдельный
    параграф), которые после срезания буллет-маркера (•, -, * и т.п.)
    и пробелов становятся пустыми.

    Найдено эмпирически (диагностика на k_h20231_updated (1).docx,
    параграф "יכולת מקצועית"): список буллетов напечатан через
    Shift+Enter внутри ОДНОГО параграфа; после последнего реального
    пункта в документе остался ещё один <w:br/> с голым маркером "• "
    без текста после него. python-docx превращает это в
    "...מוזיקה\\n• " — и этот хвост попадал в извлечённый текст как
    отдельный "пустой" пункт, а затем как пустой буллет в экспорте.

    Строки без "\\n" (обычный однострочный параграф) возвращаются без
    изменений — функция не трогает текст, если делить нечего.
    Если ВСЕ строки параграфа пустые после очистки — возвращается
    пустая строка (вызывающий код должен такой item отбросить целиком).
    """
    if NL not in text:
        return text
    ...
    kept = []
    for line in lines:
        residue = _BULLET_STRIP_RE.sub('', line).strip()
        if residue:
            kept.append(line)
    return NL.join(kept)
...
def _extract_structured(doc):
    ...
    После извлечения para.text применяется _strip_empty_bullet_lines
    (Cycle B) — убирает пустые "хвосты"-буллеты, возникающие из-за
    <w:br/> без текста после него. Если после очистки от параграфа
    ничего не остаётся (весь параграф состоял только из пустых
    буллет-маркеров) — такой item в результат не попадает, как и
    раньше происходило с изначально пустыми параграфами.
    """
    items = []
    for para in doc.paragraphs:
        if para.text.strip():
            cleaned = _strip_empty_bullet_lines(para.text.strip())
            if cleaned:
                items.append({"para": para, "text": cleaned})
    for table in doc.tables:
        seen = set()
